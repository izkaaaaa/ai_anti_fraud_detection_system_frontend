...
系统中有**两套**图片上传接口，它们完全不同，前端必须弄清楚使用场景。

### 对比总览

| | **WebSocket 视频帧** | **REST 图片上传** |
|---|---|---|
| **接口地址** | `ws://.../ws/{user_id}/{call_id}` | `POST /api/detection/upload/image` |
| **传输方式** | WebSocket 消息，base64 | multipart/form-data 文件 |
| **图片来源** | 通话中实时截取的**视频帧** | 用户主动截取的**屏幕截图** |
| **触发时机** | 实时：每积累 10 帧自动触发 | 手动：用户点击上传按钮 |
| **后端处理函数** | `detect_video_task`（人脸伪造检测） | `detect_image_task`（OCR + 环境识别） |
| **核心功能** | 检测通话人像是否为 AI 换脸/Deepfake | 从截图中识别平台类型、提取对话内容 |
| **推送结果类型** | `detection_result`（视频伪造置信度） | `environment_detected`（平台环境 + 文本对话检测） |
| **去重机制** | 帧序列天然去重 | 10 分钟内相同截图不重复检测 |
| **是否需要 call_id** | 是（路径参数） | 可选（建议传入） |

---

### 方式 A：WebSocket 视频帧（实时人像检测）

**用途**：实时检测通话画面中的人像是否为 Deepfake / AI 换脸。

**WebSocket 连接建立后，前端按固定帧率截取视频画面并发送：**
...
后端不立即返回结果（异步 Celery 处理），而是先回送 ACK：
...
**最终检测结果**通过 WebSocket 推送（类型为 `detection_result`），其中 `video_confidence` 即为该视频帧的真假人像判断结果：
...
```json
{
  ...
  "data": {
    "overall_score": 85.5,
    "video_confidence": 0.9234,
    "voice_confidence": 0.4567,
    "text_confidence": 0.7823,
    "is_fraud": true,
    "advice": "当前视频疑似 AI 换脸，请提高警惕..."
  }
}
```

**⚠️ 注意**：`video` 消息不触发 `environment_detected`，只触发人像 Deepfake 检测。前端不要期望发送视频帧后收到平台切换消息。

---

### 方式 B：REST 图片上传（OCR + 平台识别）

**用途**：用户手动截取通话界面截图，上传后端做 OCR 识别，提取平台类型、联系人信息，并检测对话中是否有诈骗文本。

**适用场景**：
- 用户刚接通电话，截了一张通话界面图 → 识别是微信 / QQ / 电话
- 用户截了一张聊天记录图 → 提取对话内容做文本欺诈检测
...
file: [图片文件，JPEG/PNG/WebP，最大 10MB]
call_id: 123  (可选)
```
...
{
  "code": 200,
  "message": "图片上传成功，正在进行截图环境识别（文字聊天将另行提取对话并检测）",
  "data": {
    "url": "https://minio:9000/fraud-detection/image/1/screenshot.jpg",
...
后端 OCR 识别完成后，通过 WebSocket 推送结果：

1. **平台识别结果**（`environment_detected`）：
```json
{
...
2. **文本检测结果**（`detection_result`，如有诈骗内容）：
```json
{
  "type": "detection_result",
  "data": {
...
│
├── 想检测对方人脸是否造假？
│   └── 发送 WebSocket {"type": "video", "data": base64Frame}（实时）
│       → 收到 detection_result.video_confidence
│
├── 想识别当前是微信 / QQ / 电话？
│   ├── 方式 1：主动上报 POST /environment?platform=xxx
│   │           → 立即收到 environment_detected（推荐）
│   │
│   └── 方式 2：截图上传 POST /upload/image
│               → OCR 自动识别平台 → 收到 environment_detected（需等 OCR）
│
└── 想检测聊天记录中是否有诈骗文本？
    └── 截图上传 POST /upload/image
        → OCR 提取对话 → 收到 detection_result.text_confidence
```
...
6. **两套图片接口不要混淆**：
   - `ws.send({type: "video"})` → 人像 Deepfake 检测，结果在 `detection_result.video_confidence`
   - `POST /upload/image` → OCR 平台识别 + 文本检测，结果在 `environment_detected` + `detection_result.text_confidence`
   - 两者互不干扰，前端按需使用。

7. **WebSocket `{"type": "video"}` 不返回 `environment_detected`**：视频帧消息只触发人像伪造检测，前端不应期望发送视频帧后收到平台切换消息。如需切换平台，调用 `POST /environment` 或上传截图。
