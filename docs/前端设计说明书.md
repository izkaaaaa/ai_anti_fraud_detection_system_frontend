...
色彩体系按「主色系 → 背景与容器 → 文字色 → 语义色 → 风险色」分层组织。

**主色系（墨绿色系）**：

| 用途 | 颜色名称 | 色值 | 使用场景 |
|------|---------|------|---------|
| 主色 | 墨绿 | `#58A183` | 按钮背景、进度条、Tab 指示器、图标（选中态）|
| 深主色 | 深墨绿 | `#1B553E` | 底部导航栏背景、AppBar、深色卡片背景 |
| 极深 | 渐变底色 | `#164A35` | 底部导航栏渐变底部色 |
| 浅辅助 | 浅绿 | `#6EE7B7` | 图标未选中态（带透明度版本）|
| Tab 选中文字 | 深绿文字 | `#2D4A3E` | Tab 标签选中文字、标题文字 |
...
| 用途 | 色值 | 说明 |
|------|------|------|
| 大标题 | `#0F1923` | 页面主标题、模块标题 |
| 正文 | `#2D4A3E` | 副标题、列表项标题 |
| 正文（次要）| `#6B7280` | 描述文字、说明文字 |
| 辅助文字 | `#9CA3AF` | 时间戳、占位提示、标签文字 |
| 链接色 | `#58A183` | 可点击文字，与主色一致 |

**语义色**：

| 用途 | 色值 | 说明 |
|------|------|------|
| 成功 | `#58A183` | 与主色一致，墨绿色，传达安全、正面 |
| 警告 | `#F59E0B` | 橙色，提醒用户注意 |
| 危险 | `#DC2626` | 红色，用于错误、高风险提示 |
| 信息 | `#3B82F6` | 蓝色，用于普通提示消息 |

**风险等级色**（检测页专用）：

| 风险等级 | 色值 | 说明 |
|---------|------|------|
| 安全 Safe | `#58A183` | 绿色，风险分数为 0 |
| 低风险 Low | `#6EE7B7` | 浅绿色，0 < 分数 ≤ 30 |
| 中风险 Medium | `#F59E0B` | 橙色，30 < 分数 ≤ 60 |
| 高风险 High | `#EA580C` | 深橙色，60 < 分数 ≤ 80 |
| 严重风险 Critical | `#DC2626` | 红色，80 < 分数 ≤ 100 |

#### 3.3.3 字体规范

| 级别 | 字号 | 行高 | 用途 | 示例 |
|------|------|------|------|------|
| 超小 | 11pt | 1.4 | 标签、时间戳、徽章 | "刚刚"、"3分钟前" |
| 小 | 12~13pt | 1.4 | 说明文字、次要描述 | "风险等级：中高" |
| 正文 | 14pt | 1.5 | 主要内容、列表文字 | 通话记录标题 |
| 副标题 | 16pt | 1.4 | 卡片标题、模块标题 | "家庭组成员" |
| 大标题 | 20pt | 1.3 | 页面主标题 | "实时检测"、"学习中心" |
| 超大标题 | 24~28pt | 1.2 | 检测状态文字、报告标题 | "正在守护中" |

**老年人模式**：`textScaler = 1.25×`，最大 `1.6×`

**字体粗细**：`w400`（正文）、`w500`（副标题）、`w600`（按钮）、`w700`（大标题）
...
|------|---|------|
| 超小间距 | 4dp | 标签内部紧凑排列 |
| 小间距 | 8dp | 元素内部（如 Icon + 文字间距）|
| 中间距 | 12dp | 列表项内部文字间距 |
| 默认间距 | 16dp | 组件之间、卡片内边距 |
| 大间距 | 20~24dp | 区块之间、页面边缘留白 |
| 超大间距 | 32dp | 大区块分割 |
...
| 层次 | 阴影参数 | 用途 |
|------|---------|------|
| 卡片浮起 | `0 2dp 8dp rgba(0,0,0,0.05)` | 列表卡片、对话框 |
| 悬浮按钮 | `0 4dp 12dp rgba(0,0,0,0.15)` | FAB、悬停元素 |
| 弹窗阴影 | `0 8dp 24dp rgba(0,0,0,0.2)` | 模态弹窗、底部弹窗 |

#### 3.3.7 按钮规范

| 类型 | 样式 | 圆角 | 高度 |
|------|------|------|------|
| 主按钮 | 实心墨绿色背景，白色文字 | 10dp | 44~48dp |
| 次按钮 | 墨绿色边框，白色/墨绿文字 | 8dp | 40dp |
| 文本按钮 | 无边框，墨绿色文字 | 无 | 36dp |
| 危险按钮 | 实心红色背景，白色文字 | 10dp | 44dp |
...
| 圆角 | 16dp |
| 内边距 | 16dp |
| 阴影 | `0 2dp 8dp rgba(0,0,0,0.05)` |
| 边框（可选）| `1dp solid rgba(0,0,0,0.05)` |
...
|------|---|
| 背景 | `#F8FAF9` |
| 聚焦边框 | `1.5dp solid #58A183` |
| 默认边框 | `1dp solid #E5E7EB` |
| 圆角 | 10dp |
| 内边距 | 14dp 水平，14dp 垂直 |
| 标签字体 | 13pt，`#6B7280` |
...
|------|---------|
| 进度环颜色 | `#58A183`（主色）|
| 进度环尺寸 | 24dp（小）/ 36dp（中）/ 48dp（大）|
| 空状态图标 | 48~80dp，颜色 `#CBD5E1` |
| 空状态文字 | 13pt，`#9CA3AF` |
...
| 动效类型 | 时长 | 缓动曲线 | 用途 |
|---------|------|---------|------|
| 页面切换 | 300ms | `Curves.easeInOut` | 路由跳转 |
| 弹窗弹出 | 300ms | `Curves.easeOut` | 底部弹窗、对话框 |
| 列表项出现 | 200ms | `Curves.easeOut` | 列表加载动画 |
| 按钮反馈 | 100ms | `Curves.easeIn` | 按下态颜色变化 |
| 风险等级变化 | 500ms | `Curves.easeInOut` | 颜色渐变过渡 |
| 波形更新 | 16ms（60fps）| 线性 | 音频波形动画 |

**风险色渐变**：`AnimatedContainer` / `ColorTween` 在 500ms 内平滑过渡

**老年人模式动画**：所有动画时长 × 1.3 倍

#### 3.3.13 响应式适配规范

| 场景 | 适配策略 |
|------|---------|
| 全面屏（刘海屏）| `SafeArea` 包裹全页面，避免内容被系统栏遮挡 |
| 平板横屏 | 大屏幕（>600dp 宽度）列表改为双列卡片布局 |
| 老年人模式 | 全局字号 1.25~1.6 倍放大 |
| 深色模式 | 当前版本未实现，统一使用浅色系（米白背景 + 墨绿主色）|
...
本节说明应用在通话场景下（如 QQ 通话、微信语音）获取麦克风音频的核心机制。

#### 4.4.1 通话音频获取机制

**背景问题**：当用户使用 QQ、微信等 VoIP 应用通话时，普通的 `AudioSource.mic` 只能录制物理麦克风采集的环境音，无法获取通话对方的语音。此外，部分国产定制 ROM（如华为、荣耀）对后台应用的音频访问做了严格限制，即使申请了麦克风权限，后台录音也可能被系统封锁。

**解决方案**：应用采用以下四组件协同机制获取通话音频：

| 组件 | 作用 |
|------|------|
| `VOICE_COMMUNICATION` 音频源 | 直接读取系统通话音频流（不是物理 MIC） |
| 无障碍服务 | 检测通话开始，自动触发录音 |
| 1像素 Activity | 骗过系统前台检测，保住音频访问权限 |
| 前台通知服务 | 确保进程不被系统省电策略杀死 |
...
└─────────────────────────────────┘
        ↓
启动 AudioRecord（音频源=VOICE_COMMUNICATION）
        ↓
┌─────────────────────────────────────────┐
│  通话进行中                               │
│  ┌─ 用户切换到其他 App ──→ 继续录音 ✓     │
...
| 音频源 | 说明 |
|--------|------|
| `AudioSource.mic`（值=1）| 录制物理麦克风环境音，后台会被国产 ROM 封锁 |
| `AudioSource.voiceCommunication`（值=6）| 专为 VoIP 通话设计的系统音频流，可直接读取通话双方声音 |

#### 4.4.4 1像素 Activity 与返回键闭环

**1像素 Activity 的职责**：保住音频权限。让荣耀/华为系统认为应用有前台界面，防止封锁 `VOICE_COMMUNICATION` 访问权限。

**返回键闭环的职责**：防止主界面抢占前台。只在用户试图切回应用主界面时触发，强制把主界面打回去。
...
启动 OnePxActivity（1像素）
        ↓
500ms 后执行 performGlobalAction(BACK)
        ↓
用户被强制返回上一个界面（QQ 通话等）
```
